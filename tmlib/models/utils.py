...
class ExperimentConnection(_Connection):
    ...
    def get_unique_ids(self, model_cls, n):
        ...
        ------
        ValueError
            when the table of `model_cls` is not distributed
        '''
        logger.debug(
            'get unique identifier for model "%s"', model_cls.__name__
        )
        self._cursor.execute('''
            SELECT nextval(%(sequence)s) FROM generate_series(1, %(n)s);
        ''', {
            'sequence': '{t}_id_seq'.format(t=model_cls.__table__.name),
            'n': n
        })
