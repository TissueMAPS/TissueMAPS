...
class Tool(object):
    ...
    def __init__(self, experiment_id, submission_id, use_spark=False):
        ...
        submission_id: int
            ID of the corresponding tool job submission
        use_spark: bool, optional
            whether processing should be performed via `Apache Spark`

        Note
        ----
        When `use_spark` is ``TRUE`` the application must be sumitted via
        `spark-submit <http://spark.apache.org/docs/latest/submitting-applications.html>`_.
        '''
    ...
    def get_feature_values_spark(self, mapobject_type_name, feature_name):
    ...
    def get_feature_values_sklearn(self, mapobject_type_name, feature_name):
        '''Selects all values from table "feature_values" for mapobjects of
...
class Classifier(Tool):
    ...
    def format_feature_data_sklearn(self, mapobject_type_name, feature_names):
        '''Loads feature values from database and bring the dataset into the
        format required by classifiers of the module:`sklearn` package.

        Parameters
        ----------
        mapobject_type_name: str
            name of the selected mapobject type, see
            :class:`tmlib.models.tm.MapobjectType`
        feature_names: List[str]
            names of selected features, see :class:`tmlib.models.tm.Feature`
    ...
    def format_feature_data_spark(self, mapobject_type_name, feature_names):
        '''Loads feature values from database and bring the dataset into the
        format required by classifiers of the module:`pyspark.ml` package.
