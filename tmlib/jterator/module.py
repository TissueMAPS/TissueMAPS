...
class ImageProcessingModule(object):
    ...
    def __init__(self, name, module_file, handles_description,
        ...
        self.output_stderror_path = None
        self.output_stdout_path = None
    ...
    def write_output_and_errors(self, stdout_file, stdout_data,
        ...
        stderr_data: str
            standard error of the module execution
        '''
        with open(self.output_stdout_path, 'w+') as output_log:
            output_log.write(stdout_data)
        with open(self.output_stderr_path, 'w+') as error_log:
    ...
    def language(self):
        ...
        '''
        self._language = pathutils.determine_language(self.module_file)
    ...
    def prepare_inputs(self, files, upstream_output, data_file, figure_file):
        '''
        ...
        ----------
        files: dict
            description of the processed job
        ...
        job_pattern = {j['name']: j['file'] for j in job['pattern']}
        ...
        for i in self.handles_description['input']:
            ...
            if i['class'] == 'parameter':
                ...
                if i['value'] in job_pattern.keys():
                    # Job items (filenames)
                    inputs[i['name']] = os.path.join(job['folder'],
                                                     job_pattern[i['value']])
                else:
        ...
        inputs['project_dir'] = self.project_dir
    ...
    def run(self, inputs, engine=None):
        ...
        if self.handles_description['output']:
            output_names = [o['name'] for o in self.handles_description['output']]
        else:
            output_names = []
