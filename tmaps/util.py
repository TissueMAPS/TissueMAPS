...
def extract_model_from_body(*model_classes, **kw):
    ...
    check_ownership : type
        If it should be checked that the authed user is owner
        of the object he requested. Default is False.
...
def extract_model_from_path(model_cls, check_ownership=False):
    def decorator(f):
        @functools.wraps(f)
        def wrapped(*args, **kwargs):
