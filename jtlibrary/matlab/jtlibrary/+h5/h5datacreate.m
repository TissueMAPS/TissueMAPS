...
if strcmp(params.type,'char') && params.size(1)==1 
    % --[MH 1408] -- hack around weird behavior with "strings" in Matlab
...
if ischar(params.type)
    switch params.type
        ...
        case 'cell'
            ...
            % (hdf5 equivalent to cell array of strings)
            params.datatypeId = H5T.copy('H5T_C_S1');
            H5T.set_size(params.datatypeId, 'H5T_VARIABLE');
        case 'char'
            % create "string" dataset
            params.datatypeId = H5T.copy('H5T_C_S1');
            H5T.set_size (params.datatypeId, 1);
        %-----------------------------------------------------
        case 'complex'
