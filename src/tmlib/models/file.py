...
class MicroscopeImageFile(Model, DateMixIn):
...
class MicroscopeMetadataFile(Model, DateMixIn):
...
class OmeXmlFile(Model, DateMixIn):
...
class ChannelImageFile(Model, DateMixIn):
    ...
    def name(self):
        ...
        self._name = CHANNEL_IMAGE_FILENAME_FORMAT.format(
...
class ProbabilityImageFile(Model, DateMixIn):
    ...
    def __repr__(self):
        return (
            '<ProbabilityImageFile(id=%r, tpoint=%r, zplane=%r, site=%r, objects=%r)>'
            % (self.id, self.tpoint, self.zplane, self.site_id, self.object_id)
        )


class IllumStatsFile(Model, DateMixIn):
